...
def setup():
    ...
    """
    run(
        'mkdir -p %(path)s; cd %(path)s; '
        'pyvenv venv; mkdir releases; mkdir media; mkdir static'
        % env
    )
    upload_template(
        '{{ project_name }}/__local_settings.py',
        '%(path)s/local_settings.py' % env,
        context=None, use_sudo=False, backup=False, mode=0770)
    clone_repo()
    checkout_latest()
...
def clone_repo():
    ...
    run(
        'cd %(path)s; '
        'git clone /home/%(user)s/git/repositories/{{ project_name }}.git repository'
        % env
    )
...
def checkout_latest():
    ...
    run(
        'cp -R %(path)s/repository %(path)s/releases/%(release)s; '
        'rm -rf %(path)s/releases/%(release)s/.git*'
        % env
    )
...
def install_requirements():
    ...
    run(
        'cd %(path)s; '
        '%(path)s/bin/pip install -r ./releases/%(release)s/requirements.txt'
        % env
    )
...
def symlink_current_release():
    ...
    with settings(warn_only=True):
        # remove all entries older than 3 releases (current/previous) + 1
        run('cd %(path)s/releases/; ls -t1 | tail -n +6 | xargs rm -r' % env)
...
def deploy_config_files():
    ...
    upload_template(
        'config/uwsgi-emperor.ini',
        '/etc/uwsgi/{{ project_name }}.ini',
        context=None, use_sudo=False, backup=False, mode=0644)
    upload_template(
        'config/{{ project_name }}.conf',
        '/etc/nginx/conf.d/{{ project_name }}.conf',
        context=env, use_sudo=True, backup=False, mode=0644)
    #upload_template(
    #    'config/celeryd.conf',
    #    '/etc/default/celeryd',
    #    context=None, use_sudo=True, backup=False, mode=0644)
